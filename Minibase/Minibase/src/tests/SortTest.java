...
class SORTDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		try {
			fscan = new FileScan("test1.in", attrType, attrSize, (short) 2, 2,
					projlist, null);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		Sort sort = null;
		try {
			sort = new Sort(attrType, (short) 2, attrSize, fscan, 1, order[0],
					REC_LEN1, SORTPGNUM);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		while (t != null) {
			...
			if (outval.compareTo(data2[count]) != 0) {
				System.err.println("outval = " + outval + "\tdata2[count] = "
						+ data2[count]);
	...
	protected boolean test2() {
		...
		try {
			fscan = new FileScan("test2.in", attrType, attrSize, (short) 1, 1,
					projlist, null);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
	...
	protected boolean test3() {
		...
		System.out
				.println(" -- Sorting in ascending order on the int field -- ");

		try {
			fscan = new FileScan("test3.in", attrType, attrSize, (short) 4, 4,
					projlist, null);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		Sort sort = null;
		try {
			sort = new Sort(attrType, (short) 4, attrSize, fscan, 3, order[0],
					4, SORTPGNUM);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		while (t != null) {
			...
			if (iout < ival) {
				System.err.println("count = " + count + " iout = " + iout
						+ " ival = " + ival);
		...
		// Sort "test3.in" on the int attribute (field 3) -- Ascending
		System.out
				.println(" -- Sorting in descending order on the float field -- ");
		...
		try {
			sort = new Sort(attrType, (short) 4, attrSize, fscan, 4, order[1],
					4, SORTPGNUM);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		while (t != null) {
			...
			if (fout > fval) {
				System.err.println("count = " + count + " fout = " + fout
						+ " fval = " + fval);
	...
	protected boolean test4() {
		...
		try {
			fscan1 = new FileScan("test4-1.in", attrType, attrSize, (short) 2,
					2, projlist, null);
			fscan2 = new FileScan("test4-2.in", attrType, attrSize, (short) 2,
					2, projlist, null);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		Sort sort2 = null;
		try {
			sort1 = new Sort(attrType, (short) 2, attrSize, fscan1, 1,
					order[0], REC_LEN1, SORTPGNUM);
			sort2 = new Sort(attrType, (short) 2, attrSize, fscan2, 1,
					order[1], REC_LEN1, SORTPGNUM);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		}
		...
		while (t1 != null) {
			...
			if (t2 == null) {
				...
				if (outval.compareTo(data2[NUM_RECORDS - count]) != 0) {
					System.err.println("outval = " + outval
							+ "\tdata2[count] = " + data2[NUM_RECORDS - count]);
