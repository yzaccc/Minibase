		...
		if (status == OK) {
			...
			for (int i = 0; (i < choice) && (status == OK); i++) {
				...
				try {
					...
					status = FAIL;
					System.err
							.println("*** Error inserting record " + i + "\n");
					e.printStackTrace();
				}
		...
		if (status == OK) {
			...
			if (status == OK) {
				if (SystemDefs.JavabaseBM.getNumUnpinnedBuffers() != SystemDefs.JavabaseBM
						.getNumBuffers()) {
					System.err
							.println("*** The heap-file scan has not unpinned "
		...
		if (status == OK) {
			...
			while (!done) {
				...
				if (!done && status == OK) {
					...
					rec.fval = (float) 7 * i; // We'll check that i==rec.ival
												// below.

					Tuple newTuple = null;
					try {
						newTuple = new Tuple(rec.toByteArray(), 0,
								rec.getRecLength());
					} catch (Exception e) {
						status = FAIL;
						System.err.println("" + e);
