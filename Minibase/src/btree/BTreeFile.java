...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		if (key instanceof StringKey) {
			...
			if (headerPage.get_keyType() != AttrType.attrVector100Dkey) {
				throw new KeyNotMatchException(null, "");
			}
		} else
			throw new KeyNotMatchException(null, "");
	...
	private KeyClass _Delete(KeyClass key, RID rid, PageId currentPageId,
			PageId parentPageId) throws IndexInsertRecException,
				...
				if (leafPage.delEntry(new KeyDataEntry(key, rid))) {
					...
					if ((4 + leafPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
						...
						if (siblingPage.redistribute(leafPage, parentPage,
							...
							return null;
						} else if ((siblingPage.available_space() + 8 /*
																	 * 2*sizeof(slot
																	 * )
																	 */) >= ((MAX_SPACE - HFPage.DPFIXED) - leafPage
								.available_space())) {
							...
							if (rightChild.getNextPage().pid != INVALID_PAGE) {
								BTLeafPage nextLeafPage = new BTLeafPage(
										rightChild.getNextPage(),
										headerPage.get_keyType());
								nextLeafPage
										.setPrevPage(leftChild.getCurPage());
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			indexPage = new BTIndexPage(pinPage(currentPageId),
					headerPage.get_keyType());
			...
			if ((4 /* sizeof slot */+ indexPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
				...
				int pushKeySize = 0;
				if (direction == 1) {
					pushKeySize = BT.getKeyLength(parentPage
							.findKey(siblingPage.getFirst(new RID()).key));
				} else if (direction == -1) {
					pushKeySize = BT.getKeyLength(parentPage.findKey(indexPage
							.getFirst(new RID()).key));
				}
