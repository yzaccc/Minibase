 ...
 * data abstraction. The BTLeafPage and BTIndexPage code know very little about
 * the various key types.
 * 
 * Essentially we provide a way for those classes to package and unpackage
 * <key,data> pairs in a space-efficient manner. That is, the packaged result
 * contains the key followed immediately by the data value; No padding bytes are
 * used (not even for alignment).
 * 
 * Furthermore, the BT<*>Page classes need not know anything about the possible
 * AttrType values, since all their processing of <key,data> pairs is done by
 * the functions declared here.
 * 
 * In addition to packaging/unpacking of <key,value> pairs, we provide a
 * keyCompare function for the (obvious) purpose of comparing keys of arbitrary
 * type (as long as they are defined here).
 * 
 * For debug utilities, we provide some methods to print any page or the whole
 * B+ tree structure or all leaf pages.
 * 
 */
public class BT implements GlobalConst {
	...
	public final static int keyCompare(KeyClass key1, KeyClass key2)
		...
		}else if ((key1 instanceof Vector100Key) && (key2 instanceof Vector100Key)) {
			return Vector100Key.keyCompare((Vector100Key)key1, (Vector100Key)key2);
		}
	 ...
	 * 
	 * @param key
	 *            specify the key whose length will be calculated. Input
	 *            parameter.
	...
	protected final static int getKeyLength(KeyClass key)
		...
		if (key instanceof StringKey) {
			...
			return ((Vector100Key) key).getDataLength();
		} else
			throw new KeyNotMatchException(null, "key types do not match");
	}
	 ...
	 * 
	 * @param pageType
	 *            NodeType.LEAF or NodeType.INDEX. Input parameter.
	 * @return return 8 if it is of NodeType.LEA; return 4 if it is of
	 *         NodeType.INDEX.
	 ...
	 * 
	 * @param key
	 *            an object of KeyClass. Input parameter.
	 * @param pageType
	 *            NodeType.LEAF or NodeType.INDEX. Input parameter.
	 ...
	 * 
	 * @param from
	 *            It's a bytes array where KeyDataEntry will come from. Input
	 *            parameter.
	 * @param offset
	 *            the offset in the bytes. Input parameter.
	 * @param keyType
	 *            It specifies the type of key. It can be AttrType.attrString or
	 *            AttrType.attrInteger. Input parameter.
	 * @param nodeType
	 *            It specifes NodeType.LEAF or NodeType.INDEX. Input parameter.
	 * @param length
	 *            The length of (key, data) in byte array "from". Input
	 *            parameter.
	 ...
	 *                error from the lower layer
	 */
	protected final static KeyDataEntry getEntryFromBytes(byte[] from,
			int offset, int length, int keyType, short nodeType)
		...
		try {
			...
			if (nodeType == NodeType.INDEX) {
				n = 4;
				data = new IndexData(Convert.getIntValue(offset + length - 4,
						from));
			...
			if (keyType == AttrType.attrInteger) {
				key = new IntegerKey(new Integer(Convert.getIntValue(offset,
						from)));
				...
				key = new StringKey(Convert.getStrValue(offset, from, length
						- n));
			} else if (keyType == AttrType.attrVector100Dkey) {
				key = null;
				try {
					key = Convert.get100DVectorKeyValue2(offset, from, length -n
							);
				} catch (Exception e) {
	 ...
	 * 
	 * @param entry
	 *            specify the data entry. Input parameter.
	...
	protected final static byte[] getBytesFromEntry(KeyDataEntry entry)
		...
		try {
			...
			if (entry.key instanceof IntegerKey) {
				Convert.setIntValue(((IntegerKey) entry.key).getKey()
						.intValue(), 0, data);
			} else if (entry.key instanceof StringKey) {
				Convert.setStrValue(((StringKey) entry.key).getKey(), 0, data);
			} else if (entry.key instanceof Vector100Key) {
				// System.out.println("in BT getBytesFromEntry "+(((Vector100Key)
				// entry.key)==null));
				...
				Convert.set100DVectorKeyValue(vkey, 0, data);
			} else
				throw new KeyNotMatchException(null, "key types do not match");

			if (entry.data instanceof IndexData) {
				Convert.setIntValue(((IndexData) entry.data).getData().pid, m,
						data);
			} else if (entry.data instanceof LeafData) {
				Convert.setIntValue(((LeafData) entry.data).getData().slotNo,
						m, data);
				Convert.setIntValue(
						((LeafData) entry.data).getData().pageNo.pid, m + 4,
						data);
	 ...
	 * 
	 * @param pageno
	 *            the number of page. Input parameter.
	 * @param keyType
	 *            It specifies the type of key. It can be AttrType.attrString or
	 *            AttrType.attrInteger. Input parameter.
	 * @exception IOException
