...
public class BufMgr implements GlobalConst {
	...
	private void privFlushPages(PageId pageid, int all_pages)
			throws HashOperationException, PageUnpinnedException,
		...
		for (i = 0; i < numBuffers; i++)
			...
			if ((all_pages != 0) || (frmeTable[i].pageNo.pid == pageid.pid)) {
				...
				{
					System.out.println("Pined page id is "+frmeTable[i].pageNo.pid);
				...
				if (frmeTable[i].dirty != false) {
					...
					try {
						...
						throw new HashOperationException(e2,
								"BUFMGR: HASH_TBL_ERROR.");
					}
	...
	public void pinPage(PageId pin_pgid, Page page, boolean emptyPage)
		...
		if (frameNo < 0) { // Not in the buffer pool
			...
			if (bst != true) {
				throw new HashOperationException(null,
...
//					System.out.println("in BufMgr debug");
					read_page(pin_pgid, apage);
				} catch (Exception e) {

					bst = hashTable.remove(frmeTable[frameNo].pageNo);
					if (bst != true)
						throw new HashOperationException(e,
								"BUFMGR: HASH_TABLE_ERROR.");

					frmeTable[frameNo].pageNo.pid = INVALID_PAGE; // frame is
																	// empty
					frmeTable[frameNo].dirty = false;
					...
					if (bst != true)
						throw new ReplacerException(e,
								"BUFMGR: REPLACER_ERROR.");

					throw new PageNotReadException(e,
							"BUFMGR: DB_READ_PAGE_ERROR.");
				}
	...
	public PageId newPage(Page firstpage, int howmany)
		...
		try {
			System.out.println("Pinpage invoke by newPage(),firstpageid is "+firstPageId);
		...
		System.out.println("new page's first page is "+firstPageId);
		return firstPageId;
