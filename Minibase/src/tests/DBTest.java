...
class DBDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		if (status == OK) {
			...
			for (int i = 0; i < 20 && status == OK; ++i) {
				...
				byte[] data = new byte[2 * writeStr.length()]; // leave enough
																// space
				try {
				...
				try {
					SystemDefs.JavabaseDB.write_page(new PageId(runStart.pid
							+ i), pg);
				} catch (Exception e) {
					status = FAIL;
	...
	protected boolean test2() {
		...
		if (status == OK) {
			System.out
					.print("  - Read stuff back from pages we wrote in test 1\n");

			for (int i = 0; i < 20 && status == OK; ++i) {
				Page pg = new Page();
				try {
					SystemDefs.JavabaseDB.read_page(
							new PageId(runStart.pid + i), pg);
				} catch (Exception e) {
					status = FAIL;
				...
				try {
					readStr = Convert.getStrValue(0, pg.getpage(),
							2 * testStr.length());
				} catch (IOException e) {
					System.err.println("" + e);
					status = FAIL;
	...
	protected boolean test3() {
		...
		if (status == OK) {
			...
			try {
				...
				if (pgid == null) { // no exception should be thrown in this
									// case
					status = FAIL;
		...
		if (status == OK) {
			...
			try {
				...
				if (pgid == null) {
					System.err
							.println("**** Looking up a nonexistent file entry");
					System.out.println("  --> Failed as expected \n");
					status = FAIL;
				}
			}
		...
		if (status == OK) {
			System.out
					.print("  - Try to add a file entry that's already there\n");
			try {
		...
		if (status == OK) {
			System.out
					.print("  - Try to add a file entry whose name is too long\n");
			...
			String name = new String(data); // make it big,just for the heck of
											// it
			try {
				SystemDefs.JavabaseDB.add_file_entry(name, new PageId(0));
			} catch (FileNameTooLongException e) {
				System.err
						.println("**** Adding a file entry with too long a name");
				System.out.println("  --> Failed as expected \n");
				status = FAIL;
			} catch (Exception e) {
				e.printStackTrace();
		...
		if (status == OK) {
			System.out
					.print("  - Try to allocate a run of pages that's too long \n");
			try {
				pgid = new PageId();
		...
		if (status == OK) {
			...
			try {
				SystemDefs.JavabaseDB.allocate_page(pgid, -10); // made up 1 to
																// test -10
			}
		...
		if (status == OK) {
			System.out
					.print("  - Try to deallocate a negative run of pages \n");
			try {
				SystemDefs.JavabaseDB.deallocate_page(pgid, -10); // made up 1
																	// to test
																	// -10
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
	...
	protected boolean test4() {
		...
		if (status == OK) {
			...
			try {
				SystemDefs.JavabaseDB.allocate_page(pgid, dbsize - 3);   //num_map_pages=2
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
				e.printStackTrace();
			}
			...
			if (status == OK) {
				if (pgid.pid != 3) {
					status = FAIL;
					System.err
							.print("*** Expected the first page allocated to "
									+ "be page 3\n");
					...
					try {
						...
						status = FAIL;
						System.err
								.println("**** Allocating one additional page");
						System.out.println("  --> Failed as expected \n");
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					if (status == OK) {
						status = FAIL;
						System.err
								.println("The expected exception was not thrown\n");
					} else {
						status = OK;
					}
				}
		...
		if (status == OK) {
			try {
				SystemDefs.JavabaseDB.deallocate_page(pd2, 8); // free page
																// 33-40
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
				status = FAIL;
		...
		if (status == OK) {
			...
			try {
				SystemDefs.JavabaseDB.allocate_page(pgid, 8); // pages 33-40
																// added
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
			}
		...
		if (status == OK) {
			System.out
					.print("  - Free two continued run of the allocated pages\n");
			try {
				SystemDefs.JavabaseDB.deallocate_page(pg11, 7); // deallo pages
																// 11-17
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
				status = FAIL;
			...
			if (status == OK) {
				try {
					SystemDefs.JavabaseDB.deallocate_page(pg18, 11); // deallocate
																		// 18-28
				} catch (java.io.IOException e) {
					System.err.println(" IOerror: " + e);
					status = FAIL;
		...
		if (status == OK) {
			...
			try {
				SystemDefs.JavabaseDB.allocate_page(pgid, 18); // reallocate
																// from 11-28
			} catch (java.io.IOException e) {
				System.err.println(" IOerror: " + e);
				status = FAIL;
		...
		if (status == OK) {
			...
			try {
				...
				status = FAIL;
				System.err
						.println("**** Allocating more pages than are now available");
				System.out.println("   --> Failed as expected \n");
			} catch (Exception e) {
				e.printStackTrace();
			...
			if (status == OK) {
				try {
					SystemDefs.JavabaseDB.allocate_page(pgid, 6); // Should
																	// work.
				} catch (java.io.IOException e) {
					System.err.println(" IOerror: " + e);
					e.printStackTrace();
		...
		if (status == OK) {
			System.out
					.print("  - At this point, all pages should be claimed.  "
			...
			try {
				...
				status = FAIL;
				System.err
						.println("**** Allocating one more page than there is");
				System.out.println("   --> Failed as expected \n");
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		if (status == OK) {
			System.out
					.print("  - Free the last two pages: this tests a boundary "
							+ "condition in the space map.\n");
			try {
				SystemDefs.JavabaseDB
						.deallocate_page(new PageId(dbsize - 2), 2);
			}
			...
			if (status == OK
					&& SystemDefs.JavabaseBM.getNumUnpinnedBuffers() != SystemDefs.JavabaseBM
							.getNumBuffers()) {
				System.err
						.print("*** The disk space manager has left pages pinned\n");
				status = FAIL;
			}
		}
