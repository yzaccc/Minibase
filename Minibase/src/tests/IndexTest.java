...
class IndexDriver extends TestDriver implements GlobalConst {
	...
	protected boolean test1() {
		...
		try {
			iscan = new IndexScan(new IndexType(IndexType.B_Index), "test1.in",
					"BTreeIndex", attrType, attrSize, 2, 2, projlist, null, 2,
					true);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		while (t != null) {
			...
			if (outval.compareTo(data2[count]) != 0) {
				...
				System.err
						.println("Test1 -- OOPS! index scan not in sorted order");
				status = FAIL;
			}
			count++;
	...
	protected boolean test2() {
		...
		try {
			iscan = new IndexScan(new IndexType(IndexType.B_Index), "test1.in",
					"BTreeIndex", attrType, attrSize, 2, 2, projlist, expr, 2,
					false);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		if (t == null) {
			System.err
					.println("Test 2 -- no record retrieved from identity search.");
			status = FAIL;
			return status;
		}
		...
		System.out.println("test2 before range");// debug
		...
		while (t != null) {
			...
			if (outval.compareTo(data2[count]) != 0) {
				...
				System.err
						.println("Test2 -- OOPS! index scan not in sorted order");
				status = FAIL;
			}
			count++;
	...
	protected boolean test3() {
		...
		try {
			iscan = new IndexScan(new IndexType(IndexType.B_Index), "test3.in",
					"BTIndex", attrType, attrSize, 4, 4, projlist, expr, 3,
					false);
		} catch (Exception e) {
			status = FAIL;
			e.printStackTrace();
		...
		while (t != null) {
			...
			if (iout < ival) {
				...
				System.err
						.println("Test3 -- OOPS! index scan not in sorted order");
				status = FAIL;
				break;
			} else if (iout > 900) {
				System.err
						.println("Test 3 -- OOPS! index scan passed high key");
				status = FAIL;
				break;
			}
	...
	protected boolean test5() {
		...
		try {
			nnscan = new RSIndexScan(new IndexType(IndexType.VAIndex),
					"test5.in", "vafile2", attrType, attrSize, 1, 1, projlist,
					null, 1, target, 100, 4);
