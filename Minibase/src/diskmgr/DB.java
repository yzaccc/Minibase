...
//				}
//			System.out.println("in DB readd page pid="+pageno.pid+"**********************\n");
	...
	public void write_page(PageId pageno, Page apage)
		...
		try {
			...
			PCounterw.increment();
			//System.out.println("in DB write page pid="+pageno.pid+" "+PCounterw.counter);
		} catch (IOException e) {
			throw new FileIOException(e, "DB file I/O error");
		}
	...
	public void allocate_page(PageId start_page_num, int runsize)
		...
		for (int i = 0; i < num_map_pages; ++i) {// start forloop01
			...
			for (; num_bits_this_page > 0 && current_run_length < run_size; ++byteptr) {// start
				...
				{
//					System.out.println("in DB2 "+mask.intValue()+" "+num_bits_this_page);//debug
...
//		System.out.println("in DB1 "+current_run_length+" "+run_size);//debug
