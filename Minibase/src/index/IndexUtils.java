...
public class IndexUtils {
	...
	public static IndexFileScan BTree_scan(CondExpr[] selects, IndexFile indFile)
			throws IOException, UnknownKeyTypeException,
		...
		if (selects[1] == null) {
			if (selects[0].type1.attrType != AttrType.attrSymbol
				...
				throw new InvalidSelectionException(
						"IndexUtils.java: Invalid selection condition");
			}
			...
			case AttrType.attrString:
				if (((StringKey) key1).getKey().compareTo(
						((StringKey) key2).getKey()) < 0) {
			...
			case AttrType.attrInteger:
				if (((IntegerKey) key1).getKey().intValue() < ((IntegerKey) key2)
						.getKey().intValue()) {
					indScan = ((BTreeFile) indFile).new_scan(key1, key2);
				} else {
					indScan = ((BTreeFile) indFile).new_scan(key2, key1);
	 ...
	 * @param type
	 *            attribute type of the selection field
	 * @param choice
	 *            first (1) or second (2) operand is the value
	...
	private static KeyClass getValue(CondExpr cd, AttrType type, int choice)
		...
		case AttrType.attrVector100Dkey:
			if (choice == 1)
				return new Vector100Key(cd.operand1.vectorkey);
			else
				return new Vector100Key(cd.operand2.vectorkey);
		case AttrType.attrReal:
			/*
			 * // need FloatKey class in bt.java if (choice == 1) return new
			 * FloatKey(new Float(cd.operand.real)); else return new
			 * FloatKey(new Float(cd.operand.real));
			 */
		default:
			throw new UnknownKeyTypeException(
					"IndexUtils.java: Only Integer and String keys are supported so far");
