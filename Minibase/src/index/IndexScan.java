...
public class IndexScan extends Iterator {
	...
	public IndexScan(IndexType index, final String relName,
		...
		default:
			throw new UnknownIndexTypeException(
					"Only BTree index is supported so far");
		...
		while (nextentry != null) {
			if (index_only) {
				...
				if (_types[_fldNum - 1].attrType == AttrType.attrInteger) {
					...
					int count = 0;
					for (int i = 0; i < _fldNum; i++) {
						if (_types[i].attrType == AttrType.attrVector100D
								|| _types[i].attrType == AttrType.attrString)
							count++;
					}
					s_sizes[0] = _s_sizes[count - 1];
					...
					try {
						...
						RID rid1 = new RID(
								new PageId(((LeafData) nextentry.data)
										.getData().pageNo.pid),
								((LeafData) nextentry.data).getData().slotNo);
						KeyDataEntryVA kdva = new KeyDataEntryVA(vkey, rid1);
						Jtuple.set100DVectkeyridFld(1, kdva);
						System.out
								.println("in IndexScan get_next index only pid "
										+ ((LeafData) nextentry.data).getData().pageNo
										+ " slot "
										+ ((LeafData) nextentry.data).getData().slotNo);
						// System.out.println()
						vkey.setAllRegionNumber();
						vkey.printAllRegionNumber();
	 ...
	 * @throws IOException
	 */
	public Tuple get_next(RID rerid) throws IndexException,
