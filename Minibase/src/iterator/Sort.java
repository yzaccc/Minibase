...
public class Sort extends Iterator implements GlobalConst {
	...
	private void setup_for_merge(int tuple_size, int n_R_runs)
		...
		// this is a patch, I am not sure whether it works well -- bingjie
		// 4/20/98
			...
			if (temp_tuple != null) {
				...
				try {
					...
					throw new SortException(e,
							"Sort.java: UnknowAttrType caught from Q.enq()");
				} catch (TupleUtilsException e) {
					throw new SortException(e,
							"Sort.java: TupleUtilsException caught from Q.enq()");
				}
	...
	private int generate_runs(int max_elems, AttrType sortFldType,
			int sortFldLen) throws IOException, SortException, UnknowAttrType,
			TupleUtilsException, JoinsException, Exception {
		Tuple tuple;
		pnode cur_node;
		...
		if (order.tupleOrder == TupleOrder.Ascending) {
			try {
				...
				throw new SortException(e,
						"Sort.java: UnknowAttrType caught from MIN_VAL()");
			} catch (Exception e) {
			...
			try {
				...
				throw new SortException(e,
						"Sort.java: UnknowAttrType caught from MAX_VAL()");
			} catch (Exception e) {
				throw new SortException(e, "MIN_VAL failed");
			}
		}
		...
		while (true) {
			...
			comp_res = TupleUtils.CompareTupleWithValue(sortFldType,
					cur_node.tuple, _sort_fld, lastElem); // need
															// tuple_utils.java
			...
			if ((comp_res < 0 && order.tupleOrder == TupleOrder.Ascending)
				...
				TupleUtils.SetValue(lastElem, cur_node.tuple, _sort_fld,
						sortFldType);
				// write tuple to output file, need io_bufs.java, type cast???
				// System.out.println("Putting tuple into run " + (run_num +
				// 1));
			...
			if (p_elems_curr_Q == 0) {
				...
				if (p_elems_other_Q == 0) {
					// other queue is also empty, no more tuples to write out,
					// done
					break; // of the while(true) loop
				} else {
					// generate one more run for all tuples in the other queue
					// close current run and start next run
					n_tuples[run_num] = (int) o_buf.flush(); // need
																// io_bufs.java
					run_num++;
					...
					// set the last Elem to be the minimum value for the sort
					// field
					if (order.tupleOrder == TupleOrder.Ascending) {
						try {
							MIN_VAL(lastElem, sortFldType);
	...
	private Tuple delete_min() throws IOException, SortException, Exception {
		...
		if (i_buf[cur_node.run_num].empty() != true) {
			...
			if (new_tuple != null) {
				 ...
				 * new_tuple.print(_in);
				 */
				cur_node.tuple = new_tuple; // no copy needed -- I think Bingjie
											// 4/22/98
				try {
					Q.enq(cur_node);
				} catch (UnknowAttrType e) {
				...
				throw new SortException(
						"********** Wait a minute, I thought input is not empty ***************");
			}
	...
	private void MIN_VAL(Tuple lastElem, AttrType sortFldType)
		...
		case AttrType.attrVector100D:
			lastElem.set100DVectFld(_sort_fld, TupleUtils.target);
			break;
	...
	private void MAX_VAL(Tuple lastElem, AttrType sortFldType)
		...
		switch (sortFldType.attrType) {
		case AttrType.attrVector100D:
			Vector100Dtype maxv = Vector100Dtype
					.getMaxVector100D(TupleUtils.target);
			lastElem.set100DVectFld(_sort_fld, maxv);
			break;
		case AttrType.attrInteger:
			// lastElem.setHdr(fld_no, junk, null);
	...
	public Sort(AttrType[] in, short len_in, short[] str_sizes, Iterator am,
			...
			Vector100Dtype target, int vk) throws IOException, SortException {
		if (vk > 0)
			_k = vk;
		else
	...
	// public Tuple get_k() throws IOException, SortException, UnknowAttrType,
	// LowMemException, JoinsException, Exception
	// {
	// if (first_time)
	// {
